...
dist = Phi4Dist(BETA_SOURCE, LAM, LATTICE_LENGTH, thermalization=PRIOR_THERM,
        discard=PRIOR_DISC)

val_dataloader = Prior(dist, sample_shape=[1, 1])

def markov_chain(acc_file, it, model, val_dataloader):
    phi_curr, weight_curr = model.forward(next(val_dataloader))
    
    for i in range(it):
        phi_prop, weight_prop = model.forward(next(val_dataloader))
